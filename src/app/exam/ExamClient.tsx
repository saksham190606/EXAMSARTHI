...
import { logExamEventAction, finishExamAction } from '@/app/actions/examActions';
...
export function ExamClient({ sessionId, questions, initialState }: ExamClientProps) {
  const router = useRouter();
  const { t } = useTranslation();
  const [isSubmitDialogOpen, setIsSubmitDialogOpen] = useState(false);

  // Focus & Blur Event Listeners for Audit Logging
  useEffect(() => {
    const handleBlur = () => {
      logExamEventAction(sessionId, 'focus_lost', { window: 'blur' });
    };
    const handlePaste = (e: ClipboardEvent) => {
      logExamEventAction(sessionId, 'paste', { target: (e.target as HTMLElement).tagName });
    };
  ...
  const { state, actions, currentQuestion } = useExamEngine(
    ...
    initialState,
    async (finalState) => {
      await finishExamAction(sessionId);
